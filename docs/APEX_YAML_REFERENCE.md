...
### 4.2 Field Validation Configuration (validation-config)

> **⚠️ IMPORTANT: Current Implementation Status**
>
> The `validation-config` syntax is **currently not implemented** in the APEX runtime engine (apex-core). While the YAML parser accepts `validation-config` sections, they are **not processed or executed** during rule evaluation.
>
> **Current Behavior:**
> - `validation-config` sections are parsed but ignored at runtime
> - Only the `condition` field in validation rules is actually executed as SpEL
> - `message` fields in `validation-config.rules[]` are treated as plain text (not SpEL)
> - Tests pass because the validation logic is not executed
>
> **Compiler Validation Issue:**
> The APEX compiler incorrectly flags `##` patterns in `message` fields as SpEL errors, when they are actually documentation text showing format patterns (e.g., "Customer ID must be in format CUST### (e.g., CUST001)").

#### Syntax Structure (Not Currently Executed)

The following syntax is accepted by the YAML parser but **not executed** by the runtime:
...
validations:
  - id: "field-validation-example"
    ...
    condition: "#customerId != null"  # ✅ This SpEL condition IS executed

    validation-config:                # ❌ This section is NOT executed
      field: "customerId"
      rules:
        - type: "regex"
          pattern: "^CUST\\d{3}$"
        ...
        - type: "range"
          min: 1
          max: 999
          message: "Customer ID number must be between 1 and 999"
        - type: "enum"
...
To implement field validation logic, use standard SpEL expressions in the `condition` field:

```yaml
rules:
  - id: "customer-id-format"
    name: "Customer ID Format Validation"
    condition: "#customerId != null && #customerId.matches('^CUST\\\\d{3}$')"
    message: "Customer ID must be in format CUST### (e.g., CUST001)"
    severity: "ERROR"

  - id: "status-enum-validation"
    name: "Status Enum Validation"
    condition: "#status == null || T(java.util.Arrays).asList('ACTIVE', 'INACTIVE', 'SUSPENDED').contains(#status)"
    message: "Status must be one of: ACTIVE, INACTIVE, SUSPENDED"
    severity: "ERROR"
```
...
The `validation-config` feature is planned for future APEX releases and will provide:
- Structured field validation rules (regex, range, enum, custom)
- Declarative validation syntax without complex SpEL expressions
- Enhanced error messaging and validation reporting
- Integration with APEX's validation framework
