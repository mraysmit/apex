...
### 9.1 Financial Services Patterns
...
This APEX YAML Syntax Reference provides comprehensive guidance for creating maintainable, efficient, and robust business rules and enrichment logic. The key to success with APEX YAML is:

1. **Start simple**: Begin with basic patterns and gradually add complexity
2. **Follow best practices**: Use proper naming, error handling, and performance optimization
3. **Test thoroughly**: Validate your configurations with comprehensive test data
4. **Document well**: Add comments and maintain clear, readable configurations
5. **Monitor performance**: Keep track of execution times and optimize as needed
...
Break complex logic into smaller, reusable pieces:

```yaml
# Base enrichment for common calculations
enrichments:
  - id: "base-trade-calculations"
    type: "calculation-enrichment"
    calculations:
      - field: "tradeValue"
        expression: "#data.quantity * #data.price"
  ...
  # Specific enrichment that builds on base calculations
  - id: "commission-calculations"
    type: "calculation-enrichment"
    condition: "#data.tradeValueUSD != null"
    calculations:
      - field: "commissionRate"
        expression: "#data.clientTier == 'PREMIUM' ? 0.0005 : 0.001"
      - field: "commission"
        expression: "#data.tradeValueUSD * #commissionRate"
```
...
# Handle different data formats
  - field: "normalizedDate"
    expression: "#data.dateString != null && #data.dateString.matches('[0-9]{4}-[0-9]{2}-[0-9]{2}') ? T(java.time.LocalDate).parse(#data.dateString) : T(java.time.LocalDate).now()"
```
...
calculations:
  - field: "debug.inputData"
    expression: "'quantity=' + #data.quantity + ', price=' + #data.price"

  - field: "debug.calculationStep1"
    expression: "#data.quantity * #data.price"

  - field: "debug.finalResult"
    expression: "#debug.calculationStep1 * #data.multiplier"

  - field: "actualResult"
    expression: "#debug.finalResult"
```
