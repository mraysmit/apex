...
public class NestedRulesChainingTest {
    ...
    void testSequentialDependency() {
        ...
        assertEquals(new BigDecimal("82000.00"), finalAmount.setScale(2), "Final amount should reflect all discounts");
    ...
    void testFluentRuleBuilderFailurePath() {
        ...
        assertNotNull(result, "Rule chain result should not be null");
        assertEquals("SUCCESS", result.getFinalOutcome(), "Basic validation should succeed");

        // Verify execution path follows failure branch
        List<String> expectedPath = Arrays.asList("CustomerTypeCheck", "BasicValidation");
        assertEquals(expectedPath, result.getExecutionPath(), "Execution path should follow failure branch");
