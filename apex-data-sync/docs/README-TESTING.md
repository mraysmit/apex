...
```bash
# Schema reading tests (classes starting with "ReadSchema")
mvn test '-Dtest=ReadSchema*' -Dsurefire.useFile=false

# Validation tests (classes ending with "ValidationTest")
mvn test '-Dtest=*ValidationTest' -Dsurefire.useFile=false

# Pipeline tests (classes ending with "PipelineTest")
mvn test '-Dtest=*PipelineTest' -Dsurefire.useFile=false

# CSV tests (classes containing "Csv")
...
> **Note**: Quotes around `-Dtest=` patterns are required in PowerShell to prevent wildcard expansion.
