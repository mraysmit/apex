 ...
 * @since 2.1.0
 */
@DisplayName("Read Schema Database Enumeration with Exclusions Test")
class ReadSchemaDatabaseEnumerationPipelineStageTestExclusions extends SyncTestBase {
    ...
    public void setUp() {
        super.setUp();
        dbName = "schema_enum_exclusions_" + System.nanoTime();
        try {
            setupTestDatabase();
        } catch (Exception e) {
    ...
    @Test
    @DisplayName("Should enumerate tables excluding temp and system tables")
    void shouldEnumerateTablesWithExclusions() throws Exception {
        logger.info("\n=== Test: Table Enumeration with Exclusions ===\n");
        ...
        ExecutionStep readSchemaStep = result.getExecutionPath().stream()
            ...
            .filter(step -> step.getName().contains("read-schema") || step.getName().contains("read-filtered"))
            .findFirst()
            .orElse(null);
        ...
        // Verify enumerated tables (should be Map for multiple tables)
        ...
        if (stepData instanceof Map) {
            ...
            logger.info("Enumerated {} tables (excluding temp and system)", tableSchemas.size());
            
            // Verify excluded tables are not present
            assertFalse(tableSchemas.containsKey("TEMP_DATA"), "Should not include TEMP_DATA");
            assertFalse(tableSchemas.containsKey("SYSTEM_LOG"), "Should not include SYSTEM_LOG");
            
            // Log enumerated tables
    ...
    private void setupTestDatabase() throws Exception {
        ...
        try (Statement stmt = testConnection.createStatement()) {
            ...
            // Create tables that should be excluded
        ...
        logger.info("Test database created with 5 tables (3 app + 2 to exclude)");
    }

    private String generateYamlConfig() {
        return """
            metadata:
              id: "test-db-enumeration-exclusions"
              name: "Database Table Enumeration with Exclusions Test"
