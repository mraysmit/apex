...
/**
 * Tests for deprecation warnings in TransformationProcessor.
 *
 * Day 5 Requirements:
...
@DisplayName("TransformationProcessor Deprecation Tests")
class TransformationProcessorDeprecationTest {

    private static final Logger logger = LoggerFactory.getLogger(TransformationProcessorDeprecationTest.class);
    private TransformationProcessor processor;

    @BeforeAll
    static void classSetUp() {
        MDC.put("testContext", "[EXPECTED] ");
        LoggerFactory.getLogger(TransformationProcessorDeprecationTest.class)
            .info("[INTENTIONAL-FAILURE-TEST-CLASS-START] TransformationProcessorDeprecationTest intentionally triggers ERROR/WARN logs");
        LoggerFactory.getLogger(TransformationProcessorDeprecationTest.class)
    ...
    static void classTearDown() {
        LoggerFactory.getLogger(TransformationProcessorDeprecationTest.class)
            .info("[INTENTIONAL-FAILURE-TEST-CLASS-END] TransformationProcessorDeprecationTest intentional error tests completed");
        MDC.remove("testContext");
    }

    @BeforeEach
    void setUp() {
        logger.info("Setting up TransformationProcessor for deprecation tests");
