...
public class MetadataValidator {
    ...
    public MetadataValidator(String basePath) {
     ...
     * This is used by ConfigurationLoader during file loading.
     *
     * @param yamlContent The YAML content as a map
     * @param filePath The file path for error reporting
     * @throws ConfigurationException if metadata is missing or invalid
     */
    @SuppressWarnings("unchecked")
    public static void validateMetadataAndThrow(Map<String, Object> yamlContent, String filePath) throws ConfigurationException {
        // Check if metadata section exists
        Object metadataObj = yamlContent.get("metadata");
        if (metadataObj == null) {
            throw new ConfigurationException("Missing required 'metadata' section in file: " + filePath);
        }

        if (!(metadataObj instanceof Map)) {
            throw new ConfigurationException("Invalid metadata section in file: " + filePath + " - must be a map/object");
        }
        ...
        for (String requiredField : REQUIRED_METADATA_FIELDS) {
            if (!metadata.containsKey(requiredField) || metadata.get(requiredField) == null) {
                throw new ConfigurationException("Missing required metadata field '" + requiredField + "' in file: " + filePath);
            }

            Object value = metadata.get(requiredField);
            if (!(value instanceof String) || ((String) value).trim().isEmpty()) {
                throw new ConfigurationException("Metadata field '" + requiredField + "' must be a non-empty string in file: " + filePath);
            }
        }
        ...
        if (!VALID_FILE_TYPES.contains(fileType)) {
            throw new ConfigurationException("Invalid file type '" + fileType + "' in file: " + filePath + ". Valid types: " + VALID_FILE_TYPES);
        }
     ...
     */
    @SuppressWarnings("unchecked")
    private void validateMetadataSection(Map<String, Object> yamlContent, ValidationResult result) {
        // Check if metadata section exists
        Object metadataObj = yamlContent.get("metadata");
        if (metadataObj == null) {
     ...
     */
    private void validateTypeSpecificContent(Map<String, Object> yamlContent, String fileType, ValidationResult result) {
        switch (fileType) {
            case "scenario":
     ...
     */
    @SuppressWarnings("unchecked")
    private void validateScenarioContent(Map<String, Object> yamlContent, ValidationResult result) {
     ...
     */
    @SuppressWarnings("unchecked")
    private void validateScenarioRegistryContent(Map<String, Object> yamlContent, ValidationResult result) {
     ...
     */
    private void validateRuleConfigContent(Map<String, Object> yamlContent, ValidationResult result) {
     ...
     */
    private void validateDatasetContent(Map<String, Object> yamlContent, ValidationResult result) {
     ...
     */
    @SuppressWarnings("unchecked")
    private void validateProcessingStages(Object stagesObj, ValidationResult result) {
     ...
     */
    private void validateStageRequiredFields(Map<String, Object> stage, int stageIndex, ValidationResult result) {
     ...
     */
    private void validateStageFailurePolicy(Map<String, Object> stage, int stageIndex, ValidationResult result) {
     ...
     */
    private void validateNoCircularDependencies(Map<String, Set<String>> stageDependencies, ValidationResult result) {
    ...
    private void validateDependencyReferences(Map<String, Set<String>> stageDependencies,
                                            Set<String> stageNames, ValidationResult result) {
